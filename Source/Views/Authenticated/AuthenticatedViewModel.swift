    ...
    func processTokens() {
        ...
        if self.state.idToken != nil {
            ...
            do {
                ...
                self.name = jwt.claims.name
                self.nric = jwt.claims.nric
                self.family_name = jwt.claims.family_name
                self.given_name = jwt.claims.given_name
                Logger.info(data: jwt.claims.nric)
